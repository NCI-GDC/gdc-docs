...
* A breakdown of each cohort by selectable clinical facets with a bar graph and table. The facets included are `Ethnicity`, `Sex at Birth`,  `Race`, `Vital_Status`, and `Age_at_Diagnosis`. A p-value (if it can be calculated from the data) that demonstrates whether the statuses are proportionally represented is displayed for the `Ethnicity`, `Sex at Birth`, and `Vital_Status` facets.
