...
|__Copy Number Segment__|__/segment_cnvs__|Allows users to search and retrieve data for segment-level copy number variations (CNVs). The data are derived from allele‐specific copy number segments. This endpoint accepts filters (e.g., chromosome, position, length) and provides information such as chromosome, start position, end position, CNV length, and CNV change categories.|
