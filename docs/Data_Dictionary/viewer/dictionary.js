(function (_, d3, fetch, Promise) {
  ...
  function Dictionary(targetEl, options) {
    ...
    if (_DICTIONARY_CONSTANTS.BROWSER_CAPABILITIES.HASH_CHANGE_EVENT) {
      window.onhashchange = function (event) {
        if (
          window.location.href.toLowerCase().indexOf("dictionary/viewer") >= 0
        ) {
  ...
  Dictionary.prototype.init = function (shouldRefresh) {
    if (typeof Dictionary._Views === "undefined") {
      console.warn(
        "Could not find the Dictionary Views - are you sure you included the views dictionary JS File.\nAborting Dictionary Init."
      );
      return;
    }
    ...
    if (_dictionary._data === null) {
      _fetchTemplate(_DICTIONARY_CONSTANTS.TEMPLATES.MAIN_DICTIONARY)
        .then(function (html) {
          ...
          _dictionary._d3Containers.rootSelection = d3.select(
            "#dictionary-inner-container"
          );

          _dictionary._d3Containers.breadcrumbs = {};
          _dictionary._d3Containers.breadcrumbs.breadcrumbSelection = d3.select(
            "#dictionary-nav-breadcrumb"
          );
          _dictionary._d3Containers.breadcrumbs.breadcrumbStackSelection =
            d3.select("#dictionary-nav-view-stack");
        })
        .then(function () {
          _dictionary.getDataFromSource().then(function (rawDictionaryData) {
            _dictionary._data = _initDictionaryData(rawDictionaryData);

            _dictionary._d3Containers.views = _getD3ViewsForDictionary(
              _dictionary,
              _.bind(_dictionary.viewListener, _dictionary)
            );
  ...
  Dictionary.prototype.viewListener = function (viewUpdateObj) {
    ...
    switch (viewUpdateObj.eventType) {
      ...
      case _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES.NAV:
        ...
        if (_.has(params, "id")) {
          data = {};
          _.assign(data, _dictionary._data.dictionaryMap[params.id], {
            terms: _.get(_dictionary._data, "terms", {}),
          });
        }
      ...
      case _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES
        .TEMPLATE_DOWNLOAD_BY_CATEGORY_REQUESTED:
        if (_.has(params, "id")) {
          _dictionary.getDictionaryTemplates(
            params.id,
            _.get(params, "excludes", null)
          );
        }
  ...
  Dictionary.prototype.getDataFromSource = function (responseType) {
    var webServiceURL =
      this._options.dataSourceBaseHost +
      _parseContextPattern(this._options.dataSourceContextPattern);

    return _fetch(webServiceURL, responseType);
  };

  Dictionary.prototype.getDictionaryTemplateURL = function (
    dictionaryID,
    dataFormat
  ) {
    var _dictionary = this,
      fileFormat =
        dataFormat || _dictionary._options.defaultTemplateDownloadFormat,
      webServiceURL =
        _dictionary._options.dataSourceBaseHost +
        _parseContextPattern(
          _DICTIONARY_CONSTANTS.END_POINT.CONTEXT_TEMPLATE_PATTERN,
          { dictionary_name: dictionaryID }
        );

    return webServiceURL + "?format=" + fileFormat;
  };

  Dictionary.prototype.getDictionaryTemplate = function (
    dictionaryID,
    dataFormat
  ) {
    var _dictionary = this,
  ...
  Dictionary.prototype.getDictionaryTemplates = function (
    categories,
    excludes,
    dataFormat
  ) {
  ...
  Dictionary.prototype.setDefaultDictionaryTemplateDownloadFormat = function (
    downloadFormat
  ) {
    this._options.defaultTemplateDownloadFormat = downloadFormat;
  };

  Dictionary.prototype.getDefaultDictionaryTemplateDownloadFormat =
    function () {
  ...
  Dictionary.prototype._getView = function (viewName) {
    ...
    if (
      !_dictionary._d3Containers.views ||
      !_.has(
        _dictionary._d3Containers.views,
        _DICTIONARY_CONSTANTS.VIEWS.TABLE._ID
      ) ||
      !_.has(
        _dictionary._d3Containers.views[_DICTIONARY_CONSTANTS.VIEWS.TABLE._ID],
        viewName
      )
    ) {
      return null;
    ...
    return _dictionary._d3Containers.views[
      _DICTIONARY_CONSTANTS.VIEWS.TABLE._ID
    ][viewName].view;
  };
  ...
  Dictionary.prototype.updateBreadcrumb = function () {
    var _dictionary = this,
      ...
      breadcrumbSelection =
        _dictionary._d3Containers.breadcrumbs.breadcrumbSelection,
      breadcrumbStack =
        _dictionary._d3Containers.breadcrumbs.breadcrumbStackSelection,
  ...
  Dictionary.prototype.triggerViewEventFromURL = function (event) {
    ...
    var viewEvent = new Dictionary._ViewUpdateObject(
      view,
  ...
  if (typeof window.Dictionary !== "undefined") {
    throw new Error(
      "Dictionary app looks to be already defined or is in use by someone else - Aborting...."
    );
  }
  ...
  var _DICTIONARY_CONSTANTS = {
    ...
    ENTITY_LIST_DICTIONARY_KEY_ORDER: [
      "case",
      "clinical",
      "biospecimen",
      "submittable_data_file",
      "generated_data_file",
      "annotation",
      "administrative",
      "analysis",
    ...
    CATEGORY_TEMPLATE_DOWNLOAD_BLACKLIST: [
      "tbd",
      "administrative",
      "index_file",
      "analysis",
    ...
    PROPERTY_EXCLUDES: [
      "type",
      "clinical_data_bundles",
      "biospecimen_data_bundles",
      "pathology_data_bundles",
      "batch_id",
    ],
    CATEGORY_TEMPLATE_INCLUDES: {},
    END_POINT: {
      DEFAULT_URL: "https://api.gdc.cancer.gov", // TODO: env variable? 'http://localhost:5000'
      CONTEXT_PROGRAM_PROJECT_PATTERN:
        "/v0/submission/${program}/${project}/_dictionary/${dictionary_name}",
  ...
  var _defaultOptions = {
    ...
    defaultTemplateDownloadFormat:
      _DICTIONARY_CONSTANTS.END_POINT.ENDPOINT_PARAMS.TEMPLATE.TSV_TYPE,
  };
  ...
  function _getD3ViewsForDictionary(dictionary, actionCallbackFn) {
    ...
    views[_DICTIONARY_CONSTANTS.VIEWS.TABLE._ID][
      _DICTIONARY_CONSTANTS.VIEWS.TABLE.ENTITY_LIST
    ] = {
      el: tableViews.summary,
      view: new Dictionary._Views.TableEntityListView(
        tableViews.summary,
        dictionary,
        actionCallbackFn,
        dictionary
    ...
    views[_DICTIONARY_CONSTANTS.VIEWS.TABLE._ID][
      _DICTIONARY_CONSTANTS.VIEWS.TABLE.TERM_DEFINITION
    ] = {
      el: tableViews.detailed,
      view: new Dictionary._Views.TableDefinitionsView(
        tableViews.detailed,
        dictionaryData,
        actionCallbackFn,
        dictionary
  ...
  function _scrollTo(id) {
    var isSmoothScrollSupported =
        _DICTIONARY_CONSTANTS.BROWSER_CAPABILITIES.SMOOTH_SCROLL,
  ...
  function _parseContextPattern(contextPattern, patternMapping) {
    ...
    patternMapping.program =
      patternMapping.program || _DICTIONARY_CONSTANTS.END_POINT.DEFAULT_PROGRAM;
    patternMapping.project =
      patternMapping.project || _DICTIONARY_CONSTANTS.END_POINT.DEFAULT_PROJECT;
    patternMapping.dictionary_name =
      typeof patternMapping.dictionary_name === "string"
        ? patternMapping.dictionary_name
        : _DICTIONARY_CONSTANTS.END_POINT.DEFAULT_DICTIONARY;
    ...
    for (var i = 0; i < patterns.length; i++) {
      ...
      if (
        contextPattern.indexOf(tokenPattern) >= 0 &&
        _.isString(patternMapping[pattern])
      ) {
        ...
        newContextPattern = newContextPattern.replace(
          tokenPattern,
  ...
  function _fetchTemplate(templateFile, forceReload) {
    ...
    var promise = _fetch(
      _DICTIONARY_CONSTANTS.APP_ABSOLUTE_DIR +
        _DICTIONARY_CONSTANTS.TEMPLATES.RELATIVE_DIR +
        "/" +
        templateFile,
      _DICTIONARY_CONSTANTS.DATA_FORMATS.TEXT_FORMAT
    );
  ...
  function _initDictionaryData(data) {
    if (!data || _.isEmpty(data) || !_.has(data, "_definitions")) {
      console.warn(
        "Dictionary endpoint has returned invalid dictionary data! Dictionary data population aborting...",
        "Received Data: ",
        data
      );
      return null;
    ...
    var r = Object.keys(dictDataList).reduce(
      function (acc, dictionaryTitle) {
                  ...
                  (acc, key) =>
                    Object.assign(
                      ...
                      (unfilteredDict.deprecated || []).includes(key) &&
                        ![]
                          .concat(
                            unfilteredDict.required || [],
                            unfilteredDict.preferred || []
                          )
                          .includes(key)
                        ? {}
                        : {
        ...
        if (dictionaryTitle[0] === "_") {
          ...
          if (
            dictionary.category === "data_file" ||
            dictionary.category === "metadata_file"
          ) {
            if (dictionary.submittable) {
              dictionary.ui_category = "sumbittable_data_file";
              acc.dictionaryMapByCategory.submittable_data_file =
                acc.dictionaryMapByCategory.submittable_data_file.concat(
                  dictionary
                );
            } else {
              dictionary.ui_category = "generated_data_file";
              acc.dictionaryMapByCategory.generated_data_file =
                acc.dictionaryMapByCategory.generated_data_file.concat(
                  dictionary
                );
            }
          } else {
      ...
      {
        dictionaries: [],
        dictionaryMap: dictDataList,
        dictionaryMapByCategory: {
          submittable_data_file: [],
          generated_data_file: [],
        },
      }
