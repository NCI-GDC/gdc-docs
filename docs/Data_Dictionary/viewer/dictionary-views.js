(function (_DICTIONARY_CONSTANTS, _) {
  ...
  if (typeof window.Dictionary !== "function") {
    console.warn(
      "Could not find the Dictionary Global - are you sure you included the main dictionary JS File.\nAborting View Init."
    );
    return;
  }
  ...
  Dictionary._Views.TableDefinitionsView = (function () {
    function TableDefinitionsView() {
      ...
      _tableDefinitionView._parentViewName =
        _DICTIONARY_CONSTANTS.VIEWS.TABLE._ID;
      _tableDefinitionView._name =
        _DICTIONARY_CONSTANTS.VIEWS[
          _tableDefinitionView._parentViewName
        ].TERM_DEFINITION;
      ...
      _tableDefinitionView.setDictionaryData(
        _tableDefinitionView._dictionaryData
      );
    }
    ...
    TableDefinitionsView.prototype.renderView = function () {
      ...
      if (_tableDefinitionView._dictionaryData) {
        _tableDefinitionView.renderDefinitionView(
          _tableDefinitionView._dictionaryData
        );
      }
    ...
    TableDefinitionsView.prototype.renderHeader = function () {
      var _tableDefinitionView = this;
      var headerSelection = _tableDefinitionView._d3ContainerSelection
        .append("h1")
        .text(_tableDefinitionView.getPrettyName());

      var definitionControlsSelection = headerSelection
        .append("div")
        .classed("definition-controls-container", true);

      // Exclude the below from download
      var excludeCategories =
        _DICTIONARY_CONSTANTS.CATEGORY_TEMPLATE_DOWNLOAD_BLACKLIST;

      if (
        excludeCategories.indexOf(
          _tableDefinitionView._dictionaryData.ui_category.toLowerCase()
        ) < 0
      ) {
        var updateHREFFunction = function () {
          d3.select(this).attr(
            "href",
            _tableDefinitionView._parentDictionary.getDictionaryTemplateURL(
              _tableDefinitionView._dictionaryData.id
            )
          );
        };
        ...
        definitionControlsSelection
          ...
          .attr(
            "title",
            "Download the " +
              _tableDefinitionView.getPrettyName() +
              " template."
          )
          .classed(
            "dictionary-control-bttn dictionary-template-download-bttn",
            true
          )
          .html(
            '<span aria-hidden="true" class="fa fa-cloud-download"></span> &nbsp;Download Template'
          );
          ...
          .then(function (html) {
            ...
            definitionControlsSelection
              ...
              .on("click", function () {
                ...
                var dataFormat =
                    option.attr("data-format") ||
                    _DICTIONARY_CONSTANTS.ENDPOINT_PARAMS.TSV_FORMAT,
                  dataFormatLabelVal = dataFormat.toUpperCase();

                viewDataFormatLabel.text(dataFormatLabelVal);

                _tableDefinitionView._parentDictionary.setDefaultDictionaryTemplateDownloadFormat(
                  dataFormat
                );
              });
          });
    ...
    TableDefinitionsView.prototype.renderSummaryTable = function () {
      ...
      var summaryTableContainerSel = _tableDefinitionView._d3ContainerSelection
        .append("div")
        .classed(
          "dictionary-summary-table-container dictionary-definition-container",
          true
        );
    ...
    TableDefinitionsView.prototype.renderLinksTable = function () {
      ...
      var linksTableContainerSel = _tableDefinitionView._d3ContainerSelection
        .append("div")
        .classed(
          "dictionary-links-table-container dictionary-definition-container",
          true
        );
    ...
    TableDefinitionsView.prototype.renderPropertiesTable = function () {
      ...
      var propertiesTableContainerSel =
        _tableDefinitionView._d3ContainerSelection
          .append("div")
          .classed(
            "dictionary-properties-table-container dictionary-definition-container",
            true
          );
              ...
              value = {
                propertyName:
                  normalizedPropertyName === "enum"
                    ...
                    : propertyName,
                propertyValue:
                  propertyVal === "boolean" ? ["true", "false"] : propertyVal,
              };
              break;
            case "anyof":
            case "oneof":
              value = {
                propertyName:
                  normalizedPropertyName === "oneof" ? "One of" : "Any of",
                propertyValue: [],
              };
    ...
    function _prepPropertiesTableData(dictionaryData) {
      ...
      excludeProperties = excludeProperties.concat(
        _DICTIONARY_CONSTANTS.PROPERTY_EXCLUDES
      );

      // Exclude unique keys
      if (_.isArray(dictionaryData.uniqueKeys)) {
        var uniqueKeys = _.reduce(
          dictionaryData.uniqueKeys,
          function (flattenedKeys, keys) {
      ...
      if (links !== null) {
        if (links.topLevelLinks.length) {
          excludeProperties = excludeProperties.concat(
            _.map(links.topLevelLinks, function (l) {
              return l[0].name;
            })
          );
        }
        ...
        if (links.subLinks.length) {
          ...
          var justSublinkNames = _.map(sublinks, function (sublinkIDObjs) {
            ...
            return _.reduce(
              _.first(sublinkIDObjs),
              function (sublinksArray, l) {
          ...
          var flattenedSublinkIDs = _.reduce(
            justSublinkNames,
            function (propertyList, names) {
        ...
        if (links.excludedLinks.length) {
          ...
          var flattenedExcludedLinks = _.reduce(
            excludedLinks,
            function (list, link) {
              list.push(link.name);
              return list;
      ...
      var requiredPartition = _.partition(
        _.keys(dictionaryProperties),
        function (propertyName) {
          return (
            requiredProperties && requiredProperties.indexOf(propertyName) >= 0
          );
        }
      );

      if (requiredPartition.length === 2) {
        propertyIDs = _.sortBy(requiredPartition[0]).concat(
          _.sortBy(requiredPartition[1])
        );
      } else {
      ...
      for (var i = 0; i < propertyIDs.length; i++) {
        var p = [],
          ...
          description =
            _.get(property, "term.description", false) ||
            _.get(property, "description", false) ||
            _.get(dictionaryTerms, propertyName + ".description", "n/a"),
          valueOrType = _getPropertyValueOrType(property),
          CDE =
            _.get(dictionaryTerms, propertyName + ".node.property.termDef") ||
    ...
    function _renderPropertiesTable(
      _tableDefinitionView,
      ...
      if (dataRows === null) {
        tableContainerSelection
          ...
          .html(
            '<i class="fa fa-gears"></i> &nbsp;No Properties for this entity.'
          );

        return;
      }

      var definitionTable = tableContainerSelection
        .append("table")
      ...
      var tRows = tBody.selectAll("tr").data(dataRows).enter().append("tr");
      ...
      tRows
        ...
        .html(function (d, i) {
            ...
            case 0:
              if (_.isString(data)) {
                ...
                data =
                  '<div id="' +
                  data +
                  '"><a class="monospace dictionary-anchor"  href="#?view=' +
                  _tableDefinitionView.getViewName() +
                  "&id=" +
                  dictionaryData.id +
                  "&anchor=" +
                  data +
          ...
          data =
            '<div class="values-accordion"><ul class="bullets monospace"><li>' +
            data.propertyName +
            ": <ul>" +
            bullets +
            "</ul></li></ul></div>";
    ...
    function _renderSummaryTable(
      ...
      var dictionaryData = _tableDefinitionView._dictionaryData,
        uniqueKeys = _.get(dictionaryData, "uniqueKeys", [
          _DICTIONARY_CONSTANTS.DATA_FORMATS.MISSING_VAL,
        ]);
    ...
    function createLinkData(link) {
      ...
      if (!_.has(link, "name")) {
        return _.times(
          4,
          _.constant(_DICTIONARY_CONSTANTS.DATA_FORMATS.MISSING_VAL)
        );
      }

      var linkID = _.get(link, "target_type"),
        linkLabel = link.label
          ? link.label.split("_").join(" ")
          : _valueOrDefault();

      linkData.push({ id: linkID, name: link.name });
      linkData.push(link.name);
      linkData.push(
        _capitalizeWords(_valueOrDefault(link.backref).split("_").join(" ")) +
          " " +
          "<strong>" +
          _capitalizeWords(linkLabel) +
          "</strong> " +
          _capitalizeWords(
            _valueOrDefault(link.target_type.split("_").join(" "))
          )
      );
    ...
    function _renderLinksTable(_tableDefinitionView, tableContainerSelection) {
      ...
      if (
        dataRows === null ||
        (dataRows.links.length === 0 && dataRows.subLinks.length === 0)
      ) {
        tableContainerSelection
          .append("h3")
      ...
      tHead
        ...
        .html(function (d, i) {
          if (i === 1) {
            return (
              '<span class="dictionary-tooltip"><em>' +
              d +
            ...
            case 0:
              ...
              link = _.map(link, function (l) {
                return (
                  ...
                  " " +
                  _capitalizeWords(
                    _capitalizeWords(l.id.split("_").join(" "))
                  ) +
                  "</a>"
                );
              }).join("<br />\n");
  ...
  Dictionary._Views.TableEntityListView = (function () {
    function TableEntityListView() {
      ...
      _tableEntityListView._parentViewName =
        _DICTIONARY_CONSTANTS.VIEWS.TABLE._ID;
      _tableEntityListView._name =
        _DICTIONARY_CONSTANTS.VIEWS[
          _tableEntityListView._parentViewName
        ].ENTITY_LIST;
    ...
    TableEntityListView.prototype.renderView = function () {
      var _tableEntityListView = this;
      var categoryMap =
        _tableEntityListView._dictionaryData.dictionaryMapByCategory;
        ...
        .then(function (html) {
          ...
          templateDefinitionControlSelection
            ...
            .on("click", function () {
              ...
              _tableEntityListView._parentDictionary.setDefaultDictionaryTemplateDownloadFormat(
                dataFormat
              );
            });
        });
      ...
      for (var i = 0; i < categoryKeys.length; i++) {
        ...
        _tableEntityListView.renderEntity(
          category,
          categoryMap[category],
          _getSortFnForCategory(category, categoryMap[category])
        );
      }

      // Print any remaining not explicitly sorted keys that may not be in the hardcoded order...
      var leftOverCategories = _.difference(
        _.keys(_tableEntityListView._dictionaryData.dictionaryMapByCategory),
        categoryKeys
      ).filter(function (key) {
      ...
      _tableEntityListView._callbackFn.call(
        null,
        new Dictionary._ViewUpdateObject(_tableEntityListView)
      );
    };

    TableEntityListView.prototype.renderEntity = function (
      category,
      categoryData,
      categorySortFn
    ) {
      ...
      var getTooltipText = function () {
        ...
        switch (_.first(categoryData).category) {
          case "case":
            tooltipText =
              "Cases must be registered in GDC before clinical, biospecimen, experiment and annotation data can be submitted.";
            break;
          case "administrative":
          case "TBD":
            tooltipText =
              "The entities listed in this category are maintained by GDC. Data submission is not applicable.";
            break;
          default:
            break;
        }
      ...
      tHeadRow
        ...
        .attr(
          "href",
          "#?view=" + _tableEntityListView._name + "&anchor=" + category
        )
        .on("click", function () {
          ...
          _tableEntityListView._callbackFn.call(
            null,
            new Dictionary._ViewUpdateObject(
              _tableEntityListView,
              _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES.INNER_NAV,
              { id: category }
            )
          );
        })
        .html(function () {
          ...
          return (
            '<span class="fa fa-book" aria-hidden="true"></span> <em>' +
            _capitalizeWords(
              _.get(
                _DICTIONARY_CONSTANTS.DICTIONARY_ENTITY_MAP,
                category.toLowerCase(),
                category
              )
            ) +
            "</em>" +
            (_.isString(tooltipText)
              ? '<span class="dictionary-tooltip-content"><i></i>' +
                tooltipText +
                '</span> &nbsp;<!-- i style="color: #ccc;" class="fa fa-info-circle"></i -->'
              : "")
          );
        });
      ...
      if (excludeCategories.indexOf(category.toLowerCase()) < 0) {
        tHeadRow
          ...
          .attr(
            "title",
            "Download All Templates for the " +
              _.get(
                _DICTIONARY_CONSTANTS.DICTIONARY_ENTITY_MAP,
                category.toLowerCase(),
                category
          ...
          .on("click", function () {
            ...
            if (oneOffDictionaries.indexOf(category) >= 0) {
              _tableEntityListView._callbackFn.call(
                ...
                new Dictionary._ViewUpdateObject(
                  _tableEntityListView,
                  _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES.TEMPLATE_DOWNLOAD_REQUESTED,
                  {
                    id: category,
                  }
                )
            ...
            var exclusions =
              category === "submittable_data_file"
                ? _tableEntityListView._dictionaryData.dictionaryMapByCategory.generated_data_file.map(
                    function (f) {
                      return f.id;
                    }
                  )
            ...
            var inclusions =
              _DICTIONARY_CONSTANTS.CATEGORY_TEMPLATE_INCLUDES[category];

            _tableEntityListView._callbackFn(
              new Dictionary._ViewUpdateObject(
                _tableEntityListView,
          ...
          .html(
            '<span aria-hidden="true" class="fa fa-cloud-download"></span>&nbsp; Download  &nbsp;'
          );
      }
        ...
        .data(
          categoryData
            .filter(function (entity) {
              return _DICTIONARY_CONSTANTS.CATEGORY_TEMPLATE_INCLUDES[
                entity.category
              ]
                ? _DICTIONARY_CONSTANTS.CATEGORY_TEMPLATE_INCLUDES[
                    entity.category
                  ].indexOf(entity.id) > -1
                : true;
            })
            .filter(function (d) {
              return _DICTIONARY_CONSTANTS.LINK_EXCLUDES.indexOf(d.id) === -1;
            })
            .sort(function (a, b) {
              return _DICTIONARY_CONSTANTS.CATEGORY_TEMPLATE_INCLUDES[
                a.category
      ...
      tRows
        ...
        .attr("href", function (data) {
          return (
            "#?view=" +
            _DICTIONARY_CONSTANTS.VIEWS[
              _tableEntityListView.getParentViewName()
            ].TERM_DEFINITION +
            "&id=" +
            data.id
          );
        ...
        .on("click", function (data) {
          ...
          _tableEntityListView._callbackFn.call(
            ...
            new Dictionary._ViewUpdateObject(
              _tableEntityListView,
              _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES.NAV,
              { id: data.id }
            )
          );
        })
  ...
  function View(
    ...
    _view.hide = function () {
      ...
      _view._d3ContainerSelection
        ...
        .style("opacity", 0);
      setTimeout(function () {
        _view._d3ContainerSelection.style("display", "none");
      }, 250);
  ...
  Dictionary._ViewUpdateObject = function (
    viewObject,
    viewEventType,
    viewParams
  ) {
    ...
    _viewObject.eventType =
      viewEventType || _DICTIONARY_CONSTANTS.VIEW_UPDATE_EVENT_TYPES.DEFAULT;
