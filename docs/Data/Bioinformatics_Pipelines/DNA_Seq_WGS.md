...
Output from the new WGS variant calling pipelines mentioned above has been released starting with [Data Release 42](LINK2ReleaseNotes), replacing the previous WGS variant calling pipelines used by the GDC since DR27. The earlier workflow is based on a workflow generated by the [Sanger Institute](https://github.com/cancerit/dockstore-cgpwgs) as the following:
